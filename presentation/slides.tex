...
Le projet comprend un CPU, une tentative de CPU pipelinée, un assembleur, et des programmes de démonstration dont une horloge temps réel.\\Le microprocesseur est simulé grâce à un simulateur de netlists en OCaml. \\Le code et la mémoire du processus partagent des espaces d'adressage séparés (architecture de Harvard). \\
Le processeur est décrit en Python à l'aide de la bibliothèque \texttt{carotte.py}, qui génère des netlists qui peuvent ensuite être simulées. Le processeur exécute une instruction complète à chaque cycle d'horloge.
...
Le processeur manipule des mots de 32 bits.
\begin{enumerate}
  \item ALU
...
\begin{frame}{Schéma du processeur}
...
\section{Assembleur et décodage d'instructions}

\begin{frame}{Assembleur}
L'assembleur est divisé en deux parties : un lexer/parser qui lit les fichiers assembleurs, et qui appelle la librairie qui donne le code binaire des instructions.
...
\begin{enumerate}
\item L'assembleur parcourt le fichier pour repérer tous les labels et mémoriser leur position (numéro de ligne de code). Les labels peuvent être nommés ou numériques.
\item L'assembleur génère le code machine instruction par instruction.
\end{enumerate}
...
L'horloge est écrit en assembleur et compilée par notre assembleur. Elle fait usage de l'ensemble des fonctionnalités de notre processeur: ALU, branchements conditionnels et inconditionnels, utilisation de nombreux registres, du multplieur, du diviseur, des labels de l'assembleur, et simulation de périphériques.
...
\begin{enumerate}
    \item[$\checkmark$] Créer un fichier assembleur .s
    \item[$\checkmark$] Compiler: python3 compiler.py fichier.s 
    \item[$\checkmark$] Simuler: python3 test\_pipeline.py
\end{enumerate}
\end{frame}
\section{Conclusion}
\begin{frame}{Conclusion}
\textbf{Ce qu'on a réalisé :}
\begin{itemize}
    \item[$\checkmark$] CPU monocycle RISC-V 32 bits fonctionnel
    \item[$\checkmark$] Multiplieur et diviseur
    \item[$\checkmark$] Assembleur complet (gestion des labels)
    \item[$\checkmark$] Horloge temps réel avec périphériques simulés
    \item[$\checkmark$] Tentative de pipeline avec forwarding et hazards
\end{itemize}

\textbf{Améliorations possibles :}
\begin{itemize}
    \item Finaliser le pipeline pour tous les programmes
    \item Ajouter cache, prédiction de branchement
\end{itemize}
\end{frame}
\end{document}
